...
{
    message("[===========================]")
    message("[<<<<< SVD_plot START >>>>>]")
    message("-----------------------------")

    # Check if bVals is not already a dataframe, convert if necessary
    ...
    ### Defind some functions to be used in SVD plot.
   ...
   # Estimate how many latent variable hidden in the data set.
    ...
    drawheatmap <- function(svdPV.m)
    {
        ...
        image(x=1:nrow(svdPV.m), y=1:ncol(svdPV.m), z=log10(svdPV.m), col=myPalette, breaks=breaks.v, xlab="", ylab="", axes=FALSE, main= "Singular Value Decomposition Analysis");
        ...
        legend(x=-(topPCA/1.5),y=3,legend=c(expression("p < 1x"~10^{-10}),expression("p < 1x"~10^{-5}),"p < 0.01", "p < 0.05", "p > 0.05"), fill=c("darkred","red","orange","pink",
        "white"),par('usr')[2], par('usr')[4], xpd=NA);
    ...
    message("SVD_plot Results will be saved in ",resultsDir," .\n")

    if(length(which(is.na(beta)))>0) message(length(which(is.na(beta)))," NA are detected in your beta Data Set, which may cause fail or uncorrect of SVD analysis. You may want to impute NA with Utils.impute() function first.")

    if(class(beta) == 'data.frame')
    {
        message("Your beta parameter is data.frame format. Utils is now changing it to matrix.")
        beta <- as.matrix(beta)
    }

    message("[SVD analysis will be proceed with ",dim(beta)[1], " probes and ",dim(beta)[2], " samples.]\n")

    message("\n[ SVD_plot() will only check the dimensions between data and targets, instead if checking if Sample_Names are correctly matched (because some user may have no Sample_Names in their targets file),thus please make sure your targets file is in accord with your data sets (beta) and (rgSet).]\n")


	################ Customise Phenotype Data ########################

    if(is.null(targets) | class(targets)=="list") stop("targets parameter in Data Frame or Matrix is necessary And must contain at least tow factors. If your targets is a list, please change its Format.")
    if(class(targets)=="matrix") targets <- as.data.frame(targets)

    PhenoTypes.lv_tmp <- targets[,!colnames(targets) %in% c("Sample_Name","Project","filenames","Basename") & apply(targets,2,function(x) length(unique(x)))!=1]
    PhenoTypes.lv <- PhenoTypes.lv_tmp

    if(!is.null(rownames(targets))) rownames(PhenoTypes.lv) <- rownames(targets)

    if(ncol(PhenoTypes.lv)>=2)
    {
        message("<< Following Factors in your targets(sample_sheet.csv) will be analysised: >>")
        sapply(colnames(PhenoTypes.lv_tmp),function(x) message("<",x,">(",class(PhenoTypes.lv[[x]]),"):",paste(head(unique(PhenoTypes.lv_tmp[,x])),collapse=", ")))
        message("[SVD_plot have automatically select ALL factors contain at least two different values from your targets(sample_sheet.csv), if you don't want to analysis some of them, please remove them manually from your targets variable then retry SVD_plot().]")
    }else
    {
    ...
    if(ncol(targets) > ncol(PhenoTypes.lv))
    {
        message("\n<< Following Factors in your targets(sample_sheet.csv) will not be analysis: >>")
        sapply(setdiff(colnames(targets),colnames(PhenoTypes.lv)),function(x) message("<",x,">"))
    ...
    if(RGEffect==TRUE & is.null(rgSet)) message("If you want to check Effect of Control Probes, you MUST provide rgSet parameter. Now SVD_plot can only analysis factors in targets.")
    ...
    if(nrow(PhenoTypes.lv)==ncol(beta)) message("\n<< PhenoTypes.lv generated successfully. >>") else stop("Dimension of your targets file (and rgSet information) is not equal to your beta matrix.")
  ...
  #Screeplot end
    if(PDFplot)
    {
        pdf(paste(resultsDir,"SVDsummary.pdf",sep=""), width=10, height=10);
        drawheatmap(svdPV.m)
        splot(svd.o,rmt.o)
        dev.off();
    }
