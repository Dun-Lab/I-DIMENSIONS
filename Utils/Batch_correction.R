...
batch_correction <- function(beta=bVals,
                            pd=targets,
                            variablename = "Sample.Type",
                            batchname = c("Sample.Type"),
                            logitTrans = TRUE)
{


    message("[===========================]")
    message("[<< Utils.RUNCOMBAT START >>]")
    message("-----------------------------")

    message("<< Preparing files for ComBat >>")
    
   if(length(which(is.na(beta)))>0) message(length(which(is.na(beta)))," NA are detected in your beta Data Set, which may cause fail or uncorrect of runCombat analysis. You may want to impute NA with Utils.impute() function first.")

    message("[Combat correction will be proceed with ",dim(beta)[1], " probes and ",dim(beta)[2], " samples.]\n")

	################ Customise Phenotype Data ########################

    if(is.null(pd) | class(pd)=="list") stop("pd parameter in Data Frame or Matrix is necessary And must contain at least two factors. If your pd is a list, please change its Format.")
    ...
    if(ncol(PhenoTypes.lv)>=1)
    {
        ...
        message("[Utils.runCombat have automatically select ALL factors contain at least two different values from your pd(sample_sheet.csv).]")
    }else
    {
    ...
    if(is.null(beta))
    {
        message("Sorry, Utils.runCombat failed. Your original dataset will be returned.")
        return(beta_2) 
    }else
    {
        message("Utils.runCombat success. Corrected dataset will be returned.")
        return(beta)
    }

    message("[<<< Utils.RUNCOMBAT END >>>]")
    message("[===========================]")
}